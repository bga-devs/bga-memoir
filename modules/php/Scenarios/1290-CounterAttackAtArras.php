<?php
namespace M44\Scenarios;

$scenarios[1290] = [
  'meta_data' => [
    'number' => '',
    'can_translate' => 'false',
    'software' => 'dow web scenario editor',
    'scenario_id' => '1290',
  ],
  'game_info' => [
    'front' => 'WESTERN',
    'operationID' => '44',
    'date_begin' => '1940-05-21',
    'date_end' => '1940-05-21',
    'type' => 'HISTORICAL',
    'starting' => 'PLAYER2',
    'side_player1' => 'AXIS',
    'side_player2' => 'ALLIES',
    'country_player1' => 'DE',
    'country_player2' => '',
    'cards_player1' => '6',
    'cards_player2' => '4',
    'victory_player1' => '6',
    'victory_player2' => '6',
  ],
  'board' => [
    'type' => 'STANDARD',
    'face' => 'COUNTRY',
    'hexagons' => [
      0 => [
        'row' => 0,
        'col' => 20,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 2,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      2 => [
        'row' => 0,
        'col' => 4,
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      3 => [
        'row' => 0,
        'col' => 8,
        'unit' => [
          'name' => 'gunger',
        ],
      ],
      4 => [
        'row' => 0,
        'col' => 10,
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      5 => [
        'row' => 0,
        'col' => 12,
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      6 => [
        'row' => 0,
        'col' => 14,
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      7 => [
        'row' => 1,
        'col' => 1,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      8 => [
        'row' => 1,
        'col' => 19,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      9 => [
        'row' => 1,
        'col' => 5,
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      10 => [
        'row' => 1,
        'col' => 7,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      11 => [
        'row' => 1,
        'col' => 15,
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      12 => [
        'row' => 1,
        'col' => 17,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      13 => [
        'row' => 2,
        'col' => 18,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      14 => [
        'row' => 2,
        'col' => 22,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      15 => [
        'row' => 2,
        'col' => 24,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      16 => [
        'row' => 2,
        'col' => 10,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      17 => [
        'row' => 3,
        'col' => 19,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      18 => [
        'row' => 3,
        'col' => 21,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      19 => [
        'row' => 3,
        'col' => 23,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      20 => [
        'row' => 3,
        'col' => 3,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      21 => [
        'row' => 3,
        'col' => 7,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      22 => [
        'row' => 3,
        'col' => 9,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      23 => [
        'row' => 3,
        'col' => 11,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      24 => [
        'row' => 3,
        'col' => 17,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      25 => [
        'row' => 4,
        'col' => 0,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      26 => [
        'row' => 4,
        'col' => 20,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      27 => [
        'row' => 4,
        'col' => 22,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      28 => [
        'row' => 4,
        'col' => 4,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      29 => [
        'row' => 4,
        'col' => 10,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      30 => [
        'row' => 4,
        'col' => 12,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      31 => [
        'row' => 5,
        'col' => 1,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      32 => [
        'row' => 5,
        'col' => 19,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      33 => [
        'row' => 5,
        'col' => 21,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      34 => [
        'row' => 5,
        'col' => 5,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      35 => [
        'row' => 5,
        'col' => 15,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      36 => [
        'row' => 6,
        'col' => 18,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      37 => [
        'row' => 6,
        'col' => 20,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      38 => [
        'row' => 6,
        'col' => 22,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      39 => [
        'row' => 6,
        'col' => 24,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      40 => [
        'row' => 6,
        'col' => 2,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      41 => [
        'row' => 6,
        'col' => 6,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      42 => [
        'row' => 7,
        'col' => 23,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      43 => [
        'row' => 7,
        'col' => 3,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      44 => [
        'row' => 7,
        'col' => 7,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      45 => [
        'row' => 7,
        'col' => 15,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      46 => [
        'row' => 7,
        'col' => 17,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      47 => [
        'row' => 8,
        'col' => 0,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      48 => [
        'row' => 8,
        'col' => 2,
        'terrain' => [
          'name' => 'hills',
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      49 => [
        'row' => 8,
        'col' => 4,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      50 => [
        'row' => 8,
        'col' => 6,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      51 => [
        'row' => 8,
        'col' => 8,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      52 => [
        'row' => 8,
        'col' => 10,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      53 => [
        'row' => 8,
        'col' => 12,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      54 => [
        'row' => 8,
        'col' => 14,
        'unit' => [
          'name' => 'gunus',
        ],
      ],
    ],
    'labels' => [
      0 => [
        'row' => 1,
        'col' => 1,
        'text' => [
          0 => 'Mercatel',
        ],
      ],
      1 => [
        'row' => 1,
        'col' => 21,
        'text' => [
          0 => 'Wailly',
        ],
      ],
      2 => [
        'row' => 3,
        'col' => 11,
        'text' => [
          0 => 'Agny',
        ],
      ],
      3 => [
        'row' => 6,
        'col' => 22,
        'text' => [
          0 => 'Dainville',
        ],
      ],
      4 => [
        'row' => 6,
        'col' => 8,
        'text' => [
          0 => 'Beaurains',
        ],
      ],
      5 => [
        'row' => 6,
        'col' => 14,
        'text' => [
          0 => 'Achicourt',
        ],
      ],
    ],
  ],
  'text' => [
    'en' => [
      'name' => clienttranslate('Counter-Attack at Arras'),
      'subtitle' => clienttranslate('Operation Frankforce'),
      'sdate' => clienttranslate('May 21, 1940'),
      'historical' => clienttranslate('Thanks to the unfolding success of their Blitz, the Germans rapidly pushed the Allied armies back, forcing the British Expeditionary Force to seize the town of Arras and dig-in, to try and hold the German tide there. But these forces were quickly swamped to, and by May 20, the city was surrounded.

In a bid to relieve the pressure on the British forces, Viscount Gort, Commander-in-Chief of the BEF, decided to counter-attack on Arras, in an advance known as Operation Frankforce. From there on, the British forces would drive south and link up the divided Allied armies. Unfortunately, the push onto Arras, originally supposed to come from a force about the size of an armor corps, fell well short of its objective; many forces simply could not be mustered, while those that did, were further divided by a futile pincer move attempt, dooming the Allied strategic goal from the start. 

On May 21st, General Erwin Rommel, in command of the 7th Panzer Division, easily repulsed the British counterattack.

The stage is set, the battle lines are drawn, and you are in command. The rest is history.'),
      'description' => clienttranslate('Axis Player
[Germany]
Take 6 Command cards.

Allied Player
[Great Britain]
Take 4 Command cards.
You move first.'),
      'rules' => clienttranslate('Blitz Rules are in effect (Actions 15 - Blitz Rules).

British Commonwealth Forces Command rules are in effect (British Commonwealth Forces).

Air Rules are optional: If used, give the Axis player one Air Sortie card and the Allied player one Air Sortie card at game start.'),
      'victory' => clienttranslate('6 Medals.'),
      'bibliography' => clienttranslate('December 2008 on-line release.'),
      'mod_date' => clienttranslate('2008-12-22T18:56:23'),
      'ownerID' => clienttranslate('6'),
    ],
  ],
];
