<?php
namespace M44\Scenarios;

$scenarios[20] = [
  'meta_data' => [
    'scenario_id' => '20',
    'status' => 'GAME',
    'can_translate' => 'false',
    'mod_date' => '2010-05-27 06:33:56',
    'ownerID' => 8860,
    'software' => 'sed 1.2',
    'create_by' => 8860,
    'create_date' => '2008-11-14 11:52:33',
    'mod_by' => 291520,
    'author_id' => 8860,
  ],
  'game_info' => [
    'front' => 'WESTERN',
    'operationID' => '30',
    'date_begin' => '1944-12-20',
    'date_end' => '1944-12-22',
    'type' => 'HISTORICAL',
    'starting' => 'PLAYER1',
    'side_player1' => 'AXIS',
    'side_player2' => 'ALLIES',
    'country_player1' => 'DE',
    'country_player2' => 'US',
    'cards_player1' => 5,
    'cards_player2' => 4,
    'victory_player1' => 6,
    'victory_player2' => 6,
    'victory_conditions' => [
      0 => [
        'standard' => [],
      ],
    ],
  ],
  'board' => [
    'type' => 'STANDARD',
    'face' => 'COUNTRY',
    'hexagons' => [
      0 => [
        'row' => 0,
        'col' => 2,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 4,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      2 => [
        'row' => 0,
        'col' => 8,
        'unit' => [
          'name' => 'tank2ger',
          'nbr_units' => 4,
        ],
      ],
      3 => [
        'row' => 0,
        'col' => 10,
        'unit' => [
          'name' => 'tank2ger',
          'nbr_units' => 4,
        ],
      ],
      4 => [
        'row' => 0,
        'col' => 12,
        'unit' => [
          'name' => 'gunger',
        ],
      ],
      5 => [
        'row' => 0,
        'col' => 16,
        'unit' => [
          'name' => 'tank2ger',
          'nbr_units' => 4,
        ],
      ],
      6 => [
        'row' => 0,
        'col' => 18,
        'unit' => [
          'name' => 'tank2ger',
          'nbr_units' => 4,
        ],
      ],
      7 => [
        'row' => 0,
        'col' => 20,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      8 => [
        'row' => 0,
        'col' => 22,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      9 => [
        'row' => 1,
        'col' => 5,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      10 => [
        'row' => 1,
        'col' => 7,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      11 => [
        'row' => 1,
        'col' => 9,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      12 => [
        'row' => 1,
        'col' => 11,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      13 => [
        'row' => 1,
        'col' => 15,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      14 => [
        'row' => 1,
        'col' => 17,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      15 => [
        'row' => 1,
        'col' => 19,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      16 => [
        'row' => 1,
        'col' => 21,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      17 => [
        'row' => 2,
        'col' => 6,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      18 => [
        'row' => 2,
        'col' => 18,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      19 => [
        'row' => 3,
        'col' => 3,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      20 => [
        'row' => 3,
        'col' => 7,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      21 => [
        'row' => 3,
        'col' => 15,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      22 => [
        'row' => 3,
        'col' => 17,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      23 => [
        'row' => 4,
        'col' => 2,
        'terrain' => [
          'name' => 'hills',
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      24 => [
        'row' => 4,
        'col' => 4,
        'terrain' => [
          'name' => 'woods',
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      25 => [
        'row' => 4,
        'col' => 6,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      26 => [
        'row' => 4,
        'col' => 16,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      27 => [
        'row' => 4,
        'col' => 20,
        'terrain' => [
          'name' => 'hills',
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      28 => [
        'row' => 4,
        'col' => 22,
        'terrain' => [
          'name' => 'hills',
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      29 => [
        'row' => 4,
        'col' => 24,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      30 => [
        'row' => 5,
        'col' => 1,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      31 => [
        'row' => 5,
        'col' => 3,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      32 => [
        'row' => 5,
        'col' => 5,
        'terrain' => [
          'name' => 'hills',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      33 => [
        'row' => 5,
        'col' => 7,
        'terrain' => [
          'name' => 'hills',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      34 => [
        'row' => 5,
        'col' => 9,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      35 => [
        'row' => 5,
        'col' => 11,
        'terrain' => [
          'name' => 'woods',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      36 => [
        'row' => 5,
        'col' => 13,
        'terrain' => [
          'name' => 'woods',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      37 => [
        'row' => 5,
        'col' => 15,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      38 => [
        'row' => 5,
        'col' => 17,
        'terrain' => [
          'name' => 'hills',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      39 => [
        'row' => 5,
        'col' => 19,
        'terrain' => [
          'name' => 'hills',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      40 => [
        'row' => 5,
        'col' => 21,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      41 => [
        'row' => 5,
        'col' => 23,
        'terrain' => [
          'name' => 'hills',
        ],
      ],
      42 => [
        'row' => 6,
        'col' => 6,
        'terrain' => [
          'name' => 'mountain',
          'behavior' => 'IMPASSABLE_HILL',
        ],
      ],
      43 => [
        'row' => 6,
        'col' => 8,
        'terrain' => [
          'name' => 'mountain',
          'behavior' => 'IMPASSABLE_HILL',
        ],
      ],
      44 => [
        'row' => 6,
        'col' => 10,
        'terrain' => [
          'name' => 'mountain',
          'behavior' => 'IMPASSABLE_HILL',
        ],
      ],
      45 => [
        'row' => 6,
        'col' => 12,
        'terrain' => [
          'name' => 'mountain',
          'behavior' => 'IMPASSABLE_HILL',
        ],
      ],
      46 => [
        'row' => 6,
        'col' => 14,
        'terrain' => [
          'name' => 'mountain',
          'behavior' => 'IMPASSABLE_HILL',
        ],
      ],
      47 => [
        'row' => 6,
        'col' => 16,
        'terrain' => [
          'name' => 'mountain',
          'behavior' => 'IMPASSABLE_HILL',
        ],
      ],
      48 => [
        'row' => 7,
        'col' => 3,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      49 => [
        'row' => 7,
        'col' => 21,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      50 => [
        'row' => 7,
        'col' => 23,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      51 => [
        'row' => 8,
        'col' => 8,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      52 => [
        'row' => 8,
        'col' => 10,
        'terrain' => [
          'name' => 'buildings',
        ],
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      53 => [
        'row' => 8,
        'col' => 12,
        'terrain' => [
          'name' => 'buildings',
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      54 => [
        'row' => 8,
        'col' => 14,
        'terrain' => [
          'name' => 'buildings',
        ],
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      55 => [
        'row' => 8,
        'col' => 16,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
    ],
    'labels' => [
      0 => [
        'row' => 4,
        'col' => 12,
        'text' => [
          0 => clienttranslate('The Prumerberg'),
        ],
      ],
      1 => [
        'row' => 7,
        'col' => 11,
        'text' => [
          0 => clienttranslate('St Vith'),
        ],
      ],
    ],
  ],
  'text' => [
    'en' => [
      'name' => clienttranslate('St Vith'),
      'subtitle' => clienttranslate('Ardennes'),
      'historical' => clienttranslate('As early as August 1944, Adolf Hitler began formulating plans for what would become the Ardennes offensive. While too ambitious for the available resources and terrain, the plan was deemed by the German High Command as having more chances of impacting the situation than throwing the same the ad-hoc divisions into the increasingly desperate Eastern front.

In the Ardennes, there were only a few cross-country roads, so forces would tend to pile up at the crossroads. St. Vith was one such town. The German tide rushed past to the north and south of St. Vith during the first days of the offensive, leaving the town to two Volksgrenadier divisions, supported by artillery and a few Tigers. In the defense, a hodge-podge force, including infantry and armor, dug in on a rugged ridge known as the Prumerberg, just east of St. Vith.

A heavy artillery barrage began the German attack. The infantry followed, advancing through gaps in the American line. The push was joined by Tiger tanks rolling up the front slopes of the Prumerberg. The American armor lay in wait on the ridge, but the Tigers, using an Eastern Front tactic of firing flares as they reached the crest, blinded the American tank crews and silhouetted the Sherman tanks. The American line broke under heavy pressure and fell back before additional forces could be deployed from the town.

The stage is set, the battle lines are drawn, and you are in command. The rest is history.'),
      'description' => clienttranslate('Axis Player: Take 5 command cards
You move first.

Allied Player: Take 4 command cards.
'),
      'victory' => clienttranslate('6 Medals'),
      'rules' => clienttranslate('The Axis Special Forces tank units have 4 figures.

The hill slope toward St Vith is impassable.'),
    ],
  ],
];
