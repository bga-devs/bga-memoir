<?php
namespace M44\Scenarios;

$scenarios[42] = [
  'meta_data' => [
    'number' => '21',
    'can_translate' => 'false',
    'software' => 'dow web scenario editor',
    'scenario_id' => '42',
  ],
  'game_info' => [
    'front' => 'WESTERN',
    'operationID' => '20',
    'date_begin' => '1944-06-06',
    'date_end' => '1944-06-06',
    'type' => 'HISTORICAL',
    'starting' => 'PLAYER2',
    'side_player1' => 'AXIS',
    'side_player2' => 'ALLIES',
    'country_player1' => 'DE',
    'country_player2' => 'US',
    'cards_player1' => '4',
    'cards_player2' => '6',
    'victory_player1' => '5',
    'victory_player2' => '5',
  ],
  'board' => [
    'type' => 'STANDARD',
    'face' => 'BEACH',
    'hexagons' => [
      0 => [
        'row' => 0,
        'col' => 20,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 2,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
      ],
      2 => [
        'row' => 0,
        'col' => 6,
        'terrain' => [
          'name' => 'river',
          'orientation' => 2,
        ],
      ],
      3 => [
        'row' => 0,
        'col' => 10,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
      ],
      4 => [
        'row' => 0,
        'col' => 16,
        'terrain' => [
          'name' => 'river',
          'orientation' => 2,
        ],
      ],
      5 => [
        'row' => 1,
        'col' => 21,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 2,
        ],
      ],
      6 => [
        'row' => 1,
        'col' => 23,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
        ],
      ],
      7 => [
        'row' => 1,
        'col' => 3,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 2,
        ],
      ],
      8 => [
        'row' => 1,
        'col' => 5,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 3,
        ],
      ],
      9 => [
        'row' => 1,
        'col' => 11,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 2,
        ],
      ],
      10 => [
        'row' => 1,
        'col' => 13,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
        ],
      ],
      11 => [
        'row' => 1,
        'col' => 15,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 3,
        ],
      ],
      12 => [
        'row' => 2,
        'col' => 0,
        'terrain' => [
          'name' => 'hills',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 1,
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      13 => [
        'row' => 2,
        'col' => 18,
        'rect_terrain' => [
          'name' => 'bunker',
          'orientation' => 1,
        ],
        'unit' => [
          'name' => 'gunger',
        ],
      ],
      14 => [
        'row' => 2,
        'col' => 24,
        'rect_terrain' => [
          'name' => 'bunker',
          'orientation' => 1,
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      15 => [
        'row' => 2,
        'col' => 6,
        'terrain' => [
          'name' => 'buildings',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      16 => [
        'row' => 3,
        'col' => 9,
        'rect_terrain' => [
          'name' => 'bunker',
          'orientation' => 1,
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      17 => [
        'row' => 3,
        'col' => 15,
        'rect_terrain' => [
          'name' => 'bunker',
          'orientation' => 1,
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      18 => [
        'row' => 4,
        'col' => 18,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      19 => [
        'row' => 4,
        'col' => 20,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      20 => [
        'row' => 4,
        'col' => 22,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      21 => [
        'row' => 4,
        'col' => 24,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      22 => [
        'row' => 4,
        'col' => 4,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      23 => [
        'row' => 4,
        'col' => 8,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      24 => [
        'row' => 4,
        'col' => 10,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      25 => [
        'row' => 4,
        'col' => 14,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      26 => [
        'row' => 4,
        'col' => 16,
        'obstacle' => [
          'name' => 'wire',
          'orientation' => 1,
        ],
      ],
      27 => [
        'row' => 5,
        'col' => 21,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      28 => [
        'row' => 5,
        'col' => 7,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      29 => [
        'row' => 5,
        'col' => 9,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      30 => [
        'row' => 5,
        'col' => 13,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      31 => [
        'row' => 5,
        'col' => 15,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      32 => [
        'row' => 5,
        'col' => 17,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      33 => [
        'row' => 6,
        'col' => 22,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      34 => [
        'row' => 6,
        'col' => 2,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      35 => [
        'row' => 6,
        'col' => 4,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      36 => [
        'row' => 6,
        'col' => 6,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      37 => [
        'row' => 6,
        'col' => 8,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      38 => [
        'row' => 6,
        'col' => 10,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      39 => [
        'row' => 6,
        'col' => 16,
        'obstacle' => [
          'name' => 'hedgehog',
        ],
      ],
      40 => [
        'row' => 7,
        'col' => 5,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      41 => [
        'row' => 7,
        'col' => 7,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      42 => [
        'row' => 7,
        'col' => 9,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      43 => [
        'row' => 7,
        'col' => 11,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      44 => [
        'row' => 7,
        'col' => 13,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      45 => [
        'row' => 8,
        'col' => 18,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      46 => [
        'row' => 8,
        'col' => 6,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      47 => [
        'row' => 8,
        'col' => 14,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      48 => [
        'row' => 8,
        'col' => 16,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
    ],
    'labels' => [
      0 => [
        'row' => 0,
        'col' => 0,
        'text' => [
          0 => 'Exit 1',
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 18,
        'text' => [
          0 => 'Exit 3',
        ],
      ],
      2 => [
        'row' => 0,
        'col' => 22,
        'text' => [
          0 => 'Marsh',
        ],
      ],
      3 => [
        'row' => 0,
        'col' => 4,
        'text' => [
          0 => 'Marsh',
        ],
      ],
      4 => [
        'row' => 0,
        'col' => 8,
        'text' => [
          0 => 'Exit 2',
        ],
      ],
      5 => [
        'row' => 0,
        'col' => 14,
        'text' => [
          0 => 'Marsh',
        ],
      ],
      6 => [
        'row' => 1,
        'col' => 7,
        'text' => [
          0 => 'La Grande Dune',
        ],
      ],
      7 => [
        'row' => 5,
        'col' => 23,
        'text' => [
          0 => 'Tare',
        ],
      ],
      8 => [
        'row' => 5,
        'col' => 11,
        'text' => [
          0 => 'Uncle',
        ],
      ],
    ],
  ],
  'text' => [
    'en' => [
      'name' => clienttranslate('Utah Beach'),
      'sdate' => clienttranslate('June 6, 1944'),
      'historical' => clienttranslate('The American plan was that following the air and naval bombardment, the 8th Regiment would land first followed by Duplex Drive (DD) tanks. Utah was divided into a northern beach \'Tare\' and a southern beach \'Uncle\'.

When Brigadier General Theodore Roosevelt Jr. landed with the first wave of infantry, he realized that it was the \'wrong\' beach. The landing crafts had drifted south however, to a section of the beach that was far less heavily defended; some credit him with saying "We\'ll start the War right here". Had the US forces landed at their intended location, they would have had to fight through a powerful defensive position.

One battalion after another came ashore with little loss of life. By the end of 6 June, the Division had achieved most of its objectives, in part because of the initiative of Brigadier General Theodore Roosevelt.

The stage is set, the battle lines are drawn, and you are in command. The rest is history.'),
      'description' => clienttranslate('Axis Player: Take 4 command cards.

Allied Player: Take 6 command cards.
You move first.'),
      'rules' => clienttranslate(
        'The Axis player is in control of the Bunkers and may claim them as a defensive position.'
      ),
      'victory' => clienttranslate('5 Medals.
An Allied unit that exits off the Axis side of the battlefield counts as one Victory Medal. The Allied unit is removed from play. Place one figure from this unit onto the Allied medal track.'),
      'bibliography' => clienttranslate(''),
      'mod_date' => clienttranslate('2008-04-07T17:35:02'),
      'ownerID' => clienttranslate('6'),
    ],
  ],
];
