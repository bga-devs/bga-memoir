<?php
namespace M44\Scenarios;

$scenarios[1412] = [
  'meta_data' => [
    'scenario_id' => '1412',
    'status' => 'APPROVED',
    'can_translate' => 'false',
    'mod_date' => '2010-06-03 07:00:46',
    'ownerID' => 8860,
    'software' => 'sed 1.2',
    'create_by' => 8860,
    'create_date' => '2008-04-07 17:51:52',
    'mod_by' => 291520,
    'author_id' => 8860,
  ],
  'game_info' => [
    'front' => 'PACIFIC',
    'date_begin' => '1942-10-23',
    'date_end' => '1942-10-23',
    'type' => 'HISTORICAL',
    'starting' => 'PLAYER1',
    'side_player1' => 'AXIS',
    'side_player2' => 'ALLIES',
    'country_player1' => 'JP',
    'country_player2' => 'US',
    'cards_player1' => 6,
    'cards_player2' => 6,
    'victory_player1' => 5,
    'victory_player2' => 5,
    'victory_conditions' => [
      0 => [
        'standard' => [],
      ],
    ],
    'options' => [
      'gung_ho' => true,
    ],
  ],
  'board' => [
    'type' => 'STANDARD',
    'face' => 'COUNTRY',
    'hexagons' => [
      0 => [
        'row' => 0,
        'col' => 14,
        'unit' => [
          'name' => 'gunjp',
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 16,
        'unit' => [
          'name' => 'gunjp',
        ],
      ],
      2 => [
        'row' => 0,
        'col' => 18,
        'unit' => [
          'name' => 'tankjp',
        ],
      ],
      3 => [
        'row' => 0,
        'col' => 20,
        'unit' => [
          'name' => 'tankjp',
        ],
      ],
      4 => [
        'row' => 1,
        'col' => 11,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      5 => [
        'row' => 1,
        'col' => 13,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      6 => [
        'row' => 1,
        'col' => 23,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      7 => [
        'row' => 2,
        'col' => 0,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      8 => [
        'row' => 2,
        'col' => 2,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      9 => [
        'row' => 2,
        'col' => 4,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      10 => [
        'row' => 2,
        'col' => 6,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      11 => [
        'row' => 2,
        'col' => 12,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      12 => [
        'row' => 2,
        'col' => 16,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      13 => [
        'row' => 2,
        'col' => 18,
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      14 => [
        'row' => 2,
        'col' => 20,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      15 => [
        'row' => 2,
        'col' => 22,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      16 => [
        'row' => 2,
        'col' => 24,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      17 => [
        'row' => 3,
        'col' => 7,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'unit' => [
          'name' => 'infjp',
        ],
      ],
      18 => [
        'row' => 3,
        'col' => 11,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      19 => [
        'row' => 3,
        'col' => 13,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      20 => [
        'row' => 3,
        'col' => 23,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      21 => [
        'row' => 4,
        'col' => 2,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      22 => [
        'row' => 4,
        'col' => 8,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 6,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      23 => [
        'row' => 4,
        'col' => 10,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      24 => [
        'row' => 4,
        'col' => 12,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      25 => [
        'row' => 4,
        'col' => 14,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      26 => [
        'row' => 4,
        'col' => 16,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 5,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      27 => [
        'row' => 4,
        'col' => 18,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      28 => [
        'row' => 4,
        'col' => 20,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      29 => [
        'row' => 4,
        'col' => 22,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 6,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      30 => [
        'row' => 4,
        'col' => 24,
        'terrain' => [
          'name' => 'pmouth',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      31 => [
        'row' => 5,
        'col' => 1,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      32 => [
        'row' => 5,
        'col' => 3,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      33 => [
        'row' => 5,
        'col' => 5,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      34 => [
        'row' => 5,
        'col' => 7,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 3,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      35 => [
        'row' => 5,
        'col' => 9,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      36 => [
        'row' => 5,
        'col' => 11,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      37 => [
        'row' => 5,
        'col' => 13,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      38 => [
        'row' => 5,
        'col' => 15,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      39 => [
        'row' => 5,
        'col' => 17,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 2,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      40 => [
        'row' => 5,
        'col' => 19,
        'terrain' => [
          'name' => 'river',
          'orientation' => 1,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      41 => [
        'row' => 5,
        'col' => 21,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 3,
          'behavior' => 'FORDABLE_STREAM',
        ],
      ],
      42 => [
        'row' => 5,
        'col' => 23,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      43 => [
        'row' => 6,
        'col' => 4,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      44 => [
        'row' => 6,
        'col' => 6,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      45 => [
        'row' => 6,
        'col' => 8,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      46 => [
        'row' => 6,
        'col' => 16,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      47 => [
        'row' => 6,
        'col' => 18,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      48 => [
        'row' => 6,
        'col' => 22,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'infus',
        ],
      ],
      49 => [
        'row' => 6,
        'col' => 24,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      50 => [
        'row' => 7,
        'col' => 7,
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'gunus',
        ],
      ],
      51 => [
        'row' => 7,
        'col' => 13,
        'terrain' => [
          'name' => 'pjungle',
        ],
      ],
      52 => [
        'row' => 7,
        'col' => 17,
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'gunus',
        ],
      ],
      53 => [
        'row' => 7,
        'col' => 21,
        'terrain' => [
          'name' => 'pjungle',
        ],
        'obstacle' => [
          'name' => 'sand',
          'orientation' => 2,
        ],
        'unit' => [
          'name' => 'gunus',
        ],
      ],
      54 => [
        'row' => 7,
        'col' => 23,
        'terrain' => [
          'name' => 'pbeach',
        ],
      ],
      55 => [
        'row' => 8,
        'col' => 8,
        'tags' => [
          0 => [
            'name' => 'tag5',
            'behavior' => 'EXIT_MARKER',
            'group' => ['E1', 'F1', 'G1', 'H1', 'I1', 'J1', 'K1', 'L1', 'M1'],
            'side' => 'AXIS',
          ],
        ],
      ],
      56 => [
        'row' => 8,
        'col' => 10,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      57 => [
        'row' => 8,
        'col' => 12,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      58 => [
        'row' => 8,
        'col' => 24,
        'terrain' => [
          'name' => 'pbeach',
        ],
        'tags' => [
          0 => [
            'name' => 'tag5',
            'behavior' => 'EXIT_MARKER',
            'group' => [],
            'side' => 'AXIS',
          ],
        ],
      ],
    ],
    'labels' => [],
  ],
  'text' => [
    'en' => [
      'name' => clienttranslate('Matanikau River'),
      'subtitle' => clienttranslate('Guadalcanal'),
      'historical' => clienttranslate('A four-pronged Japanese plan to attack and recapture Henderson Field on Guadalcanal was scheduled for October 22nd, 1942.

The main force, 7,000 men under the command of General Maruyama, planned to approach the airfield from the South, but found itself quickly bogged down in very difficult jungle terrain, forcing a two days postponement of the attack.

Unaware of the delay, the tanks and infantry of Major General Sumiyoshi, at the mouth of the Matanikau River, and Colonel Oka\'s forces, positioned upstream, crossed the river on schedule and soon found themselves stuck in the river bed, under a withering crossfire of US artillery and anti-tank gunfire that wreaked havoc on them.

On the Western flank, Lt. General "Chesty" Puller and his 1st Battalion, 7th Marines, with the 2nd Battalion, 7th Infantry in support, held ground once again. By battle\'s end, over 1000 Japanese soldiers lay dead.

The stage is set, the battle lines are drawn, and you are in command. The rest is history.
'),
      'description' => clienttranslate('Japanese Player: Take 6 Command cards.
You move first.

US Marine Player: Take 6 Command cards.
'),
      'victory' => clienttranslate('5 Medals.

Place a Japanese Objective Medal on the two hexes of the Marine baseline marked in the above set-up. A Japanese unit that exits off the Marine\'s side of the battlefield through any of the hexes in between those two Medal markers counts as a Victory Medal. Each Japanese unit that crosses through is removed from play, and one of its figures placed onto the Japanese player\'s Medal Stand.'),
      'rules' => clienttranslate('Imperial Japanese Army Command rules are in effect (see p. 7).

US Marine Corps Command rules are in effect (see p. 7).

The Matanikau River is Fordable Stream. Fordable Stream is explained on p. 6.

Jungles are explained on p. 5.

River Mouth is explained on p. 5.

'),
      'bibliography' => clienttranslate('http://en.wikipedia.org/wiki/Guadalcanal_Campaign'),
    ],
  ],
];
