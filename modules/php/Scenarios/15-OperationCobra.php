<?php
namespace M44\Scenarios;

$scenarios[15] = [
  'meta_data' => [
    'scenario_id' => '15',
    'status' => 'GAME',
    'can_translate' => 'false',
    'mod_date' => '2011-01-08 20:00:13',
    'ownerID' => 8860,
    'software' => 'sed 1.2',
    'create_by' => 8860,
    'create_date' => '2008-11-14 11:52:33',
    'mod_by' => 291520,
    'author_id' => 8860,
  ],
  'game_info' => [
    'front' => 'WESTERN',
    'date_begin' => '1944-07-25',
    'date_end' => '1944-07-26',
    'type' => 'HISTORICAL',
    'starting' => 'PLAYER2',
    'side_player1' => 'AXIS',
    'side_player2' => 'ALLIES',
    'country_player1' => 'DE',
    'cards_player1' => 4,
    'cards_player2' => 5,
    'victory_player1' => 5,
    'victory_player2' => 5,
    'victory_conditions' => [
      0 => [
        'standard' => [],
      ],
    ],
  ],
  'board' => [
    'type' => 'STANDARD',
    'face' => 'COUNTRY',
    'hexagons' => [
      0 => [
        'row' => 0,
        'col' => 12,
        'unit' => [
          'name' => 'infger',
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 14,
        'unit' => [
          'name' => 'tank2ger',
          'badge' => 'badge4',
        ],
      ],
      2 => [
        'row' => 0,
        'col' => 16,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      3 => [
        'row' => 0,
        'col' => 20,
        'terrain' => [
          'name' => 'buildings',
        ],
        'tags' => [
          0 => [
            'name' => 'medal1',
            'group' => [],
            'medal' => [
              'permanent' => false,
              'counts_for' => 1,
              'nbr_hex' => 1,
            ],
          ],
        ],
      ],
      4 => [
        'row' => 1,
        'col' => 5,
        'terrain' => [
          'name' => 'hedgerow',
        ],
      ],
      5 => [
        'row' => 1,
        'col' => 9,
        'terrain' => [
          'name' => 'buildings',
        ],
        'tags' => [
          0 => [
            'name' => 'medal1',
            'group' => [],
            'medal' => [
              'permanent' => false,
              'counts_for' => 1,
              'nbr_hex' => 1,
            ],
          ],
        ],
      ],
      6 => [
        'row' => 1,
        'col' => 13,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      7 => [
        'row' => 1,
        'col' => 15,
        'terrain' => [
          'name' => 'hedgerow',
        ],
      ],
      8 => [
        'row' => 2,
        'col' => 2,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      9 => [
        'row' => 2,
        'col' => 18,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      10 => [
        'row' => 2,
        'col' => 22,
        'terrain' => [
          'name' => 'hedgerow',
        ],
      ],
      11 => [
        'row' => 3,
        'col' => 7,
        'terrain' => [
          'name' => 'hedgerow',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      12 => [
        'row' => 3,
        'col' => 13,
        'terrain' => [
          'name' => 'hedgerow',
        ],
      ],
      13 => [
        'row' => 3,
        'col' => 17,
        'terrain' => [
          'name' => 'hedgerow',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      14 => [
        'row' => 4,
        'col' => 0,
        'terrain' => [
          'name' => 'hedgerow',
        ],
        'unit' => [
          'name' => 'tankger',
        ],
      ],
      15 => [
        'row' => 4,
        'col' => 2,
        'terrain' => [
          'name' => 'buildings',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      16 => [
        'row' => 4,
        'col' => 12,
        'terrain' => [
          'name' => 'hedgerow',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      17 => [
        'row' => 4,
        'col' => 20,
        'terrain' => [
          'name' => 'hedgerow',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      18 => [
        'row' => 4,
        'col' => 22,
        'terrain' => [
          'name' => 'hedgerow',
        ],
        'unit' => [
          'name' => 'infger',
        ],
      ],
      19 => [
        'row' => 4,
        'col' => 24,
        'terrain' => [
          'name' => 'river',
          'orientation' => 2,
        ],
      ],
      20 => [
        'row' => 5,
        'col' => 9,
        'terrain' => [
          'name' => 'hedgerow',
        ],
      ],
      21 => [
        'row' => 5,
        'col' => 23,
        'terrain' => [
          'name' => 'curve',
          'orientation' => 1,
        ],
      ],
      22 => [
        'row' => 6,
        'col' => 0,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
      ],
      23 => [
        'row' => 6,
        'col' => 6,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      24 => [
        'row' => 6,
        'col' => 8,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      25 => [
        'row' => 6,
        'col' => 16,
        'terrain' => [
          'name' => 'hedgerow',
        ],
      ],
      26 => [
        'row' => 6,
        'col' => 22,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      27 => [
        'row' => 6,
        'col' => 24,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
      ],
      28 => [
        'row' => 7,
        'col' => 1,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
        'rect_terrain' => [
          'name' => 'bridge',
          'orientation' => 3,
        ],
      ],
      29 => [
        'row' => 7,
        'col' => 3,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      30 => [
        'row' => 7,
        'col' => 5,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      31 => [
        'row' => 7,
        'col' => 7,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      32 => [
        'row' => 7,
        'col' => 9,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      33 => [
        'row' => 7,
        'col' => 11,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      34 => [
        'row' => 7,
        'col' => 13,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      35 => [
        'row' => 7,
        'col' => 15,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      36 => [
        'row' => 7,
        'col' => 19,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      37 => [
        'row' => 7,
        'col' => 21,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      38 => [
        'row' => 7,
        'col' => 23,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      39 => [
        'row' => 8,
        'col' => 0,
        'terrain' => [
          'name' => 'buildings',
        ],
      ],
      40 => [
        'row' => 8,
        'col' => 2,
        'terrain' => [
          'name' => 'river',
          'orientation' => 3,
        ],
      ],
      41 => [
        'row' => 8,
        'col' => 4,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      42 => [
        'row' => 8,
        'col' => 6,
        'terrain' => [
          'name' => 'woods',
        ],
      ],
      43 => [
        'row' => 8,
        'col' => 8,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      44 => [
        'row' => 8,
        'col' => 10,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      45 => [
        'row' => 8,
        'col' => 12,
        'unit' => [
          'name' => 'infus',
        ],
      ],
      46 => [
        'row' => 8,
        'col' => 22,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
      47 => [
        'row' => 8,
        'col' => 24,
        'unit' => [
          'name' => 'tankus',
        ],
      ],
    ],
    'labels' => [
      0 => [
        'row' => 0,
        'col' => 8,
        'text' => [
          0 => clienttranslate('St Gilles'),
        ],
      ],
      1 => [
        'row' => 0,
        'col' => 22,
        'text' => [
          0 => clienttranslate('Marigny'),
        ],
      ],
      2 => [
        'row' => 2,
        'col' => 18,
        'text' => [
          0 => clienttranslate('Monreul'),
          1 => clienttranslate('sur-Lozon'),
        ],
      ],
      3 => [
        'row' => 3,
        'col' => 3,
        'text' => [
          0 => clienttranslate('Hebecrevon'),
        ],
      ],
      4 => [
        'row' => 4,
        'col' => 24,
        'text' => [
          0 => clienttranslate('Lozon'),
          1 => clienttranslate('River'),
        ],
      ],
      5 => [
        'row' => 6,
        'col' => 0,
        'text' => [
          0 => clienttranslate('Vire'),
          1 => clienttranslate('River'),
        ],
      ],
      6 => [
        'row' => 8,
        'col' => 0,
        'text' => [
          0 => clienttranslate('Le Grand'),
          1 => clienttranslate('Hamel'),
        ],
      ],
    ],
  ],
  'text' => [
    'en' => [
      'name' => clienttranslate('Operation Cobra'),
      'historical' => clienttranslate('Operation "Cobra" was predicated on the saturation bombing of German lines over a narrow front. After the bombing, an attack would be made by three infantry divisions, with two armored and one infantry divisions held in reserve of this breakout force. General Bradley\'s intention in Cobra was to get the US forces out of "hedgerow hell" and into the uplands further south where they would be able to maneuver.

The ground attack began at 11.00 with the simple objective of seizing Marigny and St. Gilles. In the center, the bombing quickly put the old \'Panzer Lehr\' division out of action. But the western portion of the attack bogged down quickly against German defensive positions of the 13th Parachute Regiment, who had escaped most of the bombing. Around the town of Hebecrevon, Panther tanks and German infantry were also putting up a strong fight. As a result, first day gains were disappointing.

The options of "Lightning Joe" Collins, commander of the US forces, were to continue the infantry attacks in hopes of securing a clean breakthrough; or act more boldly and commit the mechanized forces held in reserves the next day. Never a timid man, he opted for the latter. By late afternoon, the 3rd Armored Division was on the outskirts of Marigny and the 2nd Armored Division moving rapidly toward St. Gilles. An attempted counter attack by Kampfgruppe Heinz and remnants of Panzer Lehr could not stop the armored advance. The US forces had achieved their breakout.

The stage is set, the battle lines are drawn, and you are in command. The rest is history.'),
      'description' => clienttranslate('Axis Player: Take 4 command cards

Allied Player: Take 5 command cards.
You move first.'),
      'victory' => clienttranslate('5 Medals
An Allied unit that captures the town of St. Gilles or Marigny counts as one Victory Medal. Place an Objective Medal in these two towns. As long as the Allied unit remains on the Town hex, it continues to count toward the Allied victory. If the unit moves off or is eliminated, it no longer counts.'),
      'rules' => clienttranslate('The Axis Special Force tank unit has 4 figures.
Place an Axis badge in the same hex as this unit to distinguish it from the other units.'),
    ],
  ],
];
